...
const resolvers = {
  Query: {
    ...
    get_ten_comments: async (_parent: any, _args: unknown, context: AuthContext) => {
      if (context.user) {
        return Feelings_Catalog.find({ user: context.user._id }).sort({ createdAt: -1 }).limit(10).populate({
          path: 'user',
          model: 'user',
        })
          .exec();
